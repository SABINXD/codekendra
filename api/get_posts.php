...
try {
    ...
    error_log("=== GET POSTS DEBUG ===");
    error_log("Current user ID: " . $current_user_id);
    
    // FIXED: Updated SQL query with proper ID handling
    $sql = "SELECT 
                p.id, 
                p.user_id, 
                p.post_text, 
    ...
    while ($row = $result->fetch_assoc()) {
        // FIXED: Validate post ID
        $post_id = (int)$row['id'];
        if ($post_id <= 0) {
            error_log("⚠️ Skipping post with invalid ID: " . $post_id);
            continue;
        }
        ...
        error_log("✅ Post added: ID=" . $post_id . ", User=" . $display_name . ", HasCode=" . (!empty($code_content) ? 'Yes' : 'No'));
    }
    
    error_log("📊 Total posts returned: " . count($posts));
